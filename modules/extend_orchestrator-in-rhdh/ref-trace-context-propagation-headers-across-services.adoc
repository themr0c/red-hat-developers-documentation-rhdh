...
* `X-TRACKER-{asterisk}`: Sanitizes and converts any custom tracking context from headers into span attributes, such as `sonataflow.tracker.{asterisk}`, to simplify querying.